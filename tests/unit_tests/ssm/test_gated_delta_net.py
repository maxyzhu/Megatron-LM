...
class TestGatedDeltaNet:
    ...
    @pytest.mark.parametrize(
        ("module", "flag_attr"),
        [
            ("gdn_in_proj", "recompute_in_proj"),
            ("gdn_conv1d", "recompute_conv1d"),
            ("gdn_gated_delta_rule", "recompute_gated_delta_rule"),
        ],
    )
    def test_selective_recompute_new_modules(self, module, flag_attr):
        """Each new GDN recompute segment: flag wired + forward/backward matches baseline.
        Runs in deterministic_mode
        """
        tp_group = parallel_state.get_tensor_model_parallel_group()
        cp_group = parallel_state.get_context_parallel_group()
        pg_collection = ProcessGroupCollection(tp=tp_group, cp=cp_group)
        ...
        rec_config.recompute_granularity = "selective"
        rec_config.recompute_modules = [module]
        ...
        torch.manual_seed(42)
        base_gdn = build_gdn(base_config)
        assert getattr(base_gdn, flag_attr) is False
        base_output, base_grads, base_input_grad = run(base_gdn, hidden_states)
        hidden_states.grad = None
        del base_gdn
        torch.cuda.empty_cache()
        ...
        torch.manual_seed(42)
        rec_gdn = build_gdn(rec_config)
        assert getattr(rec_gdn, flag_attr) is True
        rec_output, rec_grads, rec_input_grad = run(rec_gdn, hidden_states)

        rank = torch.distributed.get_rank()
        assert torch.equal(rec_output, base_output), f"Output not identical ({module=}, {rank=})"
        assert torch.equal(
            rec_input_grad, base_input_grad
        ), f"Input grad not identical ({module=}, {rank=})"
        assert set(rec_grads.keys()) == set(base_grads.keys())
        for name in base_grads:
            assert torch.equal(
                rec_grads[name], base_grads[name]
